...
/* 학생 실습 코드삽입 */
// 합성곱 신경망의 첫째 층은 28X28 픽셀의 흑백 이미지를 받는다.
// 이 층은 커널 크기가 3픽셀인 16개의 필터를 사용하고, relu 활성화 함수를 사용한다.
function createConvModel() {
  ...
  // 둘째 층에 최대 풀링층을 둔다.
  // max pooling을 이용해 주어진 영역에서 최대값을 고르며 다운 샘플링처럼 동작한다.
  model.add(tf.layers.maxPooling2d({ poolSize: 2, strides: 2 })); // strides는 kernel의 이동 횟수 명시?
  // 셋째 층은 다른 합성곱 층이다. 32개의 필터를 사용한다.
  model.add(
  ...
  // 마지막 층에 전달하기 위해 3D 텐서를 1D 벡터로 flatten  시키기.
  // (※ 마지막 분류 중에 고차원 데이터를 전달하기 위해 자주 사용하는 기법.)
