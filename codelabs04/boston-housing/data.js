...
/* 예제 작성 공간 */
// import * as addData from "https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js";

// let aD = [];
// let reader = new addData.FileReader();
// reader.onload = function () {
//   let data = reader.result;
//   let wordBook = XLSX.read(data, { type: "binary" });
//   const x = wordBook.Sheets.Data;
//   for (let j = 2; j < 335; j++) {
//     let str = String.fromCharCode(76) + j;
//     aD.push(x[str].v);
//   }
// };
/* //예제 작성 공간 */
...
/* 예제 작성 공간 */
export const transCsv = async (file) => {
  return new Promise((res) => {
    Papa.parse(file, {
      download: true,
      header: true,
      complete: (result) => {
        resolve(parseCsv(result["data"]));
      },
    });
  });
};
...
export class BostonHousingDataset {
  ...
  async loadData() {
    ...
    //   this.trainFeatures[i].push(aD[i]);
    //   this.testFeatures[i].push(aD[i]);
    // }
    let data = reader.result;
    let wordBook = XLSX.read(data, { type: "binary" });
    const x = wordBook.Sheets.Data;
