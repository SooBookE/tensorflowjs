...
실습1. 주어진 JSON 데이터를 텐서로 활용할 수 있도록 배열로 바꿔라.
방법 :
1) 업로드 : JSON
2) 임포트
3) 수작업
...
const IRIS_DATA = [];
/* 실습 작성 공간 */
import iris_data from "./iris.json" assert { type: "json" };
// import * as iris_data from "./iris.json" assert { type: "json" };
// const iris_data = JSON.stringify(data);
// require는 이미 다른 파일들과 import, export인 관계에서는 사용할 수 없다.
// fetch도 안 돼!
for (let i = 0; i < iris_data.length; i++) {
  let temp = [];
  temp.push(iris_data[i].sepalLength);
  temp.push(iris_data[i].sepalWidth);
  temp.push(iris_data[i].petalLength);
  temp.push(iris_data[i].petalWidth);
  if (iris_data[i].species == "setosa") {
    temp.push(0);
  } else if (iris_data[i].species == "versicolor") {
    temp.push(1);
  } else {
    temp.push(2);
  }
